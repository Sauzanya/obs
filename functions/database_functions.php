...
function db_connect() {
    ...
    if (!$conn) {
        error_log("Database connection failed: " . mysqli_connect_error()); // Log error to file
...
function select4LatestBook($conn) {
    $row = array();
    $query = "SELECT book_isbn, book_title, book_image FROM books ORDER BY created_at DESC LIMIT 4"; // Directly use created_at
    $result = mysqli_query($conn, $query);
    if (!$result) {
        error_log("Error fetching latest books: " . mysqli_error($conn)); // Log error
...
function getBookByIsbn($conn, $isbn) {
    ...
    if (!$stmt) {
        error_log("Error preparing statement: " . mysqli_error($conn)); // Log error
...
function getAll($conn) {
    ...
    $result = mysqli_query($conn, $query);
    if (!$result) {
        error_log("Error fetching all books: " . mysqli_error($conn)); // Log error
...
function addBook($conn, $isbn, $title, $author, $description, $price, $publisherid, $image) {
    ...
    if (!$result) {
        error_log("Error inserting book: " . mysqli_error($conn)); // Log error
...
function updateBook($conn, $isbn, $title, $author, $description, $price, $publisherid, $image) {
    ...
    $result = mysqli_stmt_execute($stmt);
    if (!$result) {
        error_log("Error updating book: " . mysqli_error($conn)); // Log error
...
function deleteBook($conn, $isbn) {
    ...
    $result = mysqli_stmt_execute($stmt);
    if (!$result) {
        error_log("Error deleting book: " . mysqli_error($conn)); // Log error
